...
## 3. 엘리베이터 제어 기능
### 3-1. 엘리베이터 권한 유저 조회 (관리자용)
유저 별로 어떤 권한을 가지고 있으며, 접근할 수 있는 층을 출력한다.
### 3-2. 엘리베이터 조작 기능
### 3-2-1. 엘리베이터 상태 조회
엘리베이터와 Mqtt 통신을 통해 현재 엘리베이터의 위치와 상태를 조회한다.
### 3-2-2. 엘리베이터 이용 정지 (관리자용)
엘리베이터 이용 여부를 변경시킨다. 정지시킬 경우 엘리베이터를 이용할 수 없다.
### 3-2-3. 엘리베이터 위치 제어
엘리베이터를 현재 위치로 호출하고, 원하는 층에 엘리베이터를 이동시키는 기능이다.<br>
이동하려는 층에 사용자가 권한이 없는 경우 이동할 수 없다.
### 3-2-4. 엘리베이터 통계 리포트 (관리자용)
엘리베이터 호출, 이동에 관련된 로그 데이터를 DB에서 조회해 출력하는 기능이다.
